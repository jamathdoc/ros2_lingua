...
Most LLM-to-robot integrations today are one-off scripts i.e. they hardcode topic names, assume a fixed sequence of actions, and break the moment anything changes. There is no standard way for a ROS 2 node to tell the world what it can do, and no safe mechanism to validate what an LLM suggests before it executes on real hardware.
...
**`ros2_lingua_core`** handles all the intelligence i.e capability definitions, the registry, the grounding engine, backward chaining, LLM backends, retry logic, and the error hierarchy. Pure Python, no ROS.

**`ros2_lingua`** wraps the core in ROS 2 nodes and services i.e the `GroundingNode` exposes the engine as a service, the `DispatcherNode` executes plans on your robot, and `LinguaMixin` lets any Python node self-register its capabilities at boot.
...
**`ros2_lingua_mock`** is a fully functional simulation layer i.e mock robot nodes, a one-line CLI, a live web dashboard, and a launch file that starts the entire system in one command. Use it to explore the API without touching real hardware.
...
- Python 3.10+
- C++17 (included with Humble's toolchain that is needed only for `ros2_lingua_cpp`)
...
A `Capability` is the fundamental unit of `ros2_lingua` i.e the structured description of one thing a node can do.
...
The `CapabilityRegistry` is the single source of truth i.e it stores all registered capabilities and the robot's current symbolic state.
...
16 integration tests exercise the full ROS 2 pipeline for grounding node, dispatcher node, state updates, and the mock robot stack.
...
Have a question, an idea, or want to show off a robot running `ros2_lingua`? Check out [Discussions](https://github.com/purahan/ros2_lingua/discussions), it's the place for anything that isn't a bug report or a concrete feature request.

If you're using `ros2_lingua` on your robot, we'd love to hear about it. Post in [Show and Tell](https://github.com/purahan/ros2_lingua/discussions/categories/show-and-tell) or open an issue.
